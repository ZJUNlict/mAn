...
void Field::leftMoveEvent(QMouseEvent *e) {
    if(pressedRobot) {
        ...
        Simulator::instance()->setRobot(rx(e->x()) / 1000.0, ry(e->y()) / 1000.0, robotID, robotTeam == PARAM::YELLOW, dir * 180 / M_PI);
    } else {
        Simulator::instance()->setBall(rx(e->x()) / 1000.0, ry(e->y()) / 1000.0);
    }
}
...
//    qDebug() << "FUCK DEBUG SIZE" << msgs.msgs_size();
    ...
    for(int i = 0; i < msgs.msgs_size(); i++) {
        ...
        case ZSS::Protocol::Debug_Msg_Debug_Type_ROBOT:
            pixmapPainter.drawEllipse(::x((msg.robot().pos().x()) * 10 - carDiameter / 2.0), ::y(-(msg.robot().pos().y()) * 10 + carDiameter / 2.0), ::w((carDiameter)), ::h(-(carDiameter)));
            break;
