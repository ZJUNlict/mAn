...
void CGlobalData::setCameraMatrix(bool real) {
    ...
    if (real) {
        ...
        saoConvertEdge();
        */
        cameraMatrix[0].fillCenter(saoConvert(CGeoPoint(3140, 69))); //(-4500,2250);
        cameraMatrix[1].fillCenter(saoConvert(CGeoPoint(-3131, 114)));
        for (int i = 0; i < PARAM::CAMERA; i = i + 1) {
            cameraMatrix[i].leftedge.min = cameraMatrix[i].campos.x() - 3000;
            cameraMatrix[i].leftedge.max = cameraMatrix[i].campos.x() - 3400;
            cameraMatrix[i].rightedge.min = cameraMatrix[i].campos.x() + 3000;
            cameraMatrix[i].rightedge.max = cameraMatrix[i].campos.x() + 3400;
            cameraMatrix[i].downedge.min = cameraMatrix[i].campos.y() - 4500;
            cameraMatrix[i].downedge.max = cameraMatrix[i].campos.y() - 4700;
            cameraMatrix[i].upedge.min = cameraMatrix[i].campos.y() + 4500;
            cameraMatrix[i].upedge.max = cameraMatrix[i].campos.y() + 4700;
        }
    } else {            //for Sim
        cameraMatrix[0].fillCenter(saoConvert(CGeoPoint(2410, 1579))); //(-4500,2250);
        cameraMatrix[1].fillCenter(saoConvert(CGeoPoint(2162, -1529))); //(-1500,2250);
